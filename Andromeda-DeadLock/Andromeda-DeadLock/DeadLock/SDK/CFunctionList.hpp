...
public:
	CBasePattern CSkeletonInstance_CalcWorldSpaceBones = { VmpStr( "CSkeletonInstance::CalcWorldSpaceBones" ) , VmpStr( "40 55 56 57 41 54 41 55 41 56 41 57 48 81 EC ? ? ? ? 48 8D 6C 24 ? 4C 8B EA" ) , CLIENT_DLL , 0 , SEARCH_TYPE_NONE };
	CBasePattern ScreenTransform = { VmpStr( "ScreenTransform" ) , VmpStr( "48 89 5C 24 ? 56 48 83 EC 20 48 8B F1 E8 ? ? ? ? F3 0F 10" ) , CLIENT_DLL , 0 , SEARCH_TYPE_NONE };
	...
	CBasePattern CCitadelInput_SetViewAngles = { VmpStr( "CCitadelInput::SetViewAngles" ) , VmpStr( "85 D2 75 ? 48 63 81 ? ? ? ? F2 41 0F 10 00 85 C0 74 ? 48 69 D0" ) , CLIENT_DLL , 0 , SEARCH_TYPE_NONE };
	CBasePattern CGameEntitySystem_GetBaseEntity = { VmpStr( "CGameEntitySystem::GetBaseEntity" ) , VmpStr( "4C 8D 49 10 81 FA FE 7F 00 00 77" ) , CLIENT_DLL };
	...
	CBasePattern GetCUserCmdTick = { VmpStr( "GetCUserCmdTick" ) , VmpStr( "40 53 48 83 EC 20 4C 8B 41 ? 48 8D 51 ? BB FF FF FF FF 4C 3B C3" ) , CLIENT_DLL , 0 , SEARCH_TYPE_NONE };
	CBasePattern GetCUserCmdArray = { VmpStr( "GetCUserCmdArray" ) , VmpStr( "48 89 4C 24 ? 41 56 41 57" ) , CLIENT_DLL , 0 , SEARCH_TYPE_NONE };
	CBasePattern GetCUserCmdBySequenceNumber = { VmpStr( "GetCUserCmdBySequenceNumber" ) , VmpStr( "44 8B CA 85 D2 7E ? B8 B5 81 4E 1B" ) , CLIENT_DLL , 0 , SEARCH_TYPE_NONE };
